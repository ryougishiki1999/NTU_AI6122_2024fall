        ...
        if self._indices_flags[index_name.value]:
            ...
            for doc_count, doc in enumerate(documents):
                ...
                if doc_count == ten_percent * percent_count:
                    ...
                    print(
                        f"Added {percent_count * 10}% = {doc_count} documents to {index_name.value} index: Time cost: {percent_end_time - percent_start_time:.2f} seconds")
                    percent_count += 1
                    percent_start_time = time.time()
            percent_end_time = time.time()
            print(
                f"Added total 100% = {doc_count} documents to {index_name.value} index: Time cost: {percent_end_time - percent_start_time:.2f} seconds")
            print("Writer starts commit")
            commit_start_time = time.time()
            writer.commit()
            commit_end_time = time.time()
            print(
                f"Writer finishes commit to {index_name.value} index: Time cost: {commit_end_time - commit_start_time:.2f} seconds")
            end_time = time.time()
